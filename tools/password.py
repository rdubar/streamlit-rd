...
def create_password(length=12, uppercase=True, lowercase=True,
    ...
    while not password_ok:
        ...
        # password should contain both upper and lower case letters & does not
        if length > 1 and (lowercase and uppercase) and (password.lower() == password):
            password_ok = False

        # password should contain at least one dash & does not
        if dashes:
            dash_found = False
            for c in dash_chars:
                if c in password:
        ...
        # password should contain a number and does not.
